...
if (!"acceptable_loss" %in% names(row)) {
  row$acceptable_loss <- row$validation_status == "pass"
}
if (!"acceptance_max_loss_ratio" %in% names(row)) {
  row$acceptance_max_loss_ratio <- NA_real_
}
if (!"acceptance_max_loss_margin" %in% names(row)) {
  row$acceptance_max_loss_margin <- NA_real_
}
fmt <- function(x) {
...
validation for image staging, template execution, result persistence, and
cleanup; the current biomedical direct-image benchmark is the PathMNIST
vignette, and the dsImaging handoff evidence is reported in the LUNG1
vignettes.
...
The committed catalogue artifact stores the output of the centralized and
...
trace above are loaded from the committed evidence artifact written by that
run.

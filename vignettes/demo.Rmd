...
```{r save}
ds.flower.save_model(result, "federated_logreg.rds")
```

## Load and predict

```{r predict}
model <- readRDS("federated_logreg.rds")
...
X <- as.matrix(new_patients)
coef <- as.vector(model$weights$coef)
intercept <- as.numeric(model$weights$intercept)

probs <- 1 / (1 + exp(-(X %*% coef + intercept)))
new_patients$probability <- round(probs, 4)
new_patients$prediction <- ifelse(probs > 0.5, 1, 0)
new_patients
```
...
file.remove("federated_logreg.rds")
```

"""Custodian-installed observer for this isolated PUBLIC segmentation campaign.
...
CONFIG_NAME = "segmentation-public-benchmark.json"
CAMPAIGN_ROOT = Path("/workspace/segmentation")
...
def load_config(allowed_root=CAMPAIGN_ROOT):
    ...
    if (not isinstance(config, dict)
            ...
            or config["dataset"] not in ("synthetic", "breast", "busbra")
...
def attach(client_app, config, observer):
    ...
    if getattr(client_app._dp_fit, "_segmentation_public_observer", False):
    ...
    def fit(model, X, y, pcfg, pins, n_staged, cfg, master, noise_multiplier, **kwargs):
        ...
        if cfg.get("loss-name") != "segmentation_bce_dice":
            raise RuntimeError("public observer is limited to segmentation")

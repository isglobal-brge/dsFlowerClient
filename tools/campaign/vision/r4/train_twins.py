#!/usr/bin/env python3
"""Fit declared R4 twins using freshly extracted TRAIN features, held in memory."""
import argparse
import hashlib
import hmac
import json
...
def main(root, run, features=None):
    controls()
    assert not (root / "r4/scoring-lock.json").exists(), "Never retrain a scored cell"
    ...
    pooled_mechanism = dp_harness.effective_dpsgd_mechanism(
        epsilon, 1e-6, 1.0, 852, pins["batch_size"], pins["local_epochs"], 5)
    ...
    for branch in ("central", "nonprivate_federated", "pooled_dp"):
        out = (root / "r4/twins/central" if branch == "central" else
               root / "r4/twins" / str(seed) / branch if branch == "nonprivate_federated" else
               run / branch)
        ...
        if branch == "central":
            ...
            for round_index in range(1, 6):
                ...
                for site, (sx, sy) in enumerate(site_data):
                    ...
                    master = hmac.new(secret,
                        f"vision-r4|{seed}|{branch}|{round_index}|{site}|{round_input_hash}".encode(),
            ...
            details = dict(seed=seed, training_pins=pins,

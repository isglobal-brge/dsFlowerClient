#!/usr/bin/env python3
"""Freeze every R4 artifact, then score the single held-out cohort once."""
import argparse
...
def main(root, verify_only):
    ...
    lock = root / "r4/scoring-lock.json"
    assert not lock.exists(), "R4 scoring already started; never repeat a scored cell"
    frozen = {}
    ...
    save(lock, dict(started_at=datetime.now(timezone.utc).isoformat(),
        token=protocol.get("execution_token", protocol.get("token")),
    ...
    for seed in protocol["seeds"]:
        ...
        for run in seed_runs:
            ...
            patient_results["trivial"] = metrics(patient_y, np.repeat(prevalence, len(patient_y)))
            ...
            record = dict(status="scored", epsilon=epsilon, seed=seed,
                ...
                patient_metric_definition="Frozen features averaged within held-out patient, modal label; same training-prevalence trivial forecast as primary image analysis. Direct fixed-numerics head inference.",

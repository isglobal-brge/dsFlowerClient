...
mkdir -p "$ROOT"/{Rlib,data_cache,logs}
...
apt-get update -qq
apt-get install -y --no-install-recommends r-base r-base-dev libcurl4-openssl-dev libssl-dev libxml2-dev libgit2-dev libfontconfig1-dev libharfbuzz-dev libfribidi-dev cmake pkgconf r-cran-arrow r-cran-digest r-cran-filelock r-cran-jsonlite r-cran-processx r-cran-ps r-cran-r6 r-cran-httr r-cran-openssl r-cran-remotes r-cran-dsi r-cran-dslite r-cran-resourcer r-cran-rnifti r-cran-reticulate r-cran-png r-cran-dbi r-cran-rsqlite r-cran-yaml r-cran-uuid r-cran-aws.s3

# Trees-only copy of ../campaign_lib.R: N-site DSLite federation plumbing, dataset preparation,
...
campaign_run_federated <- function(site_data, test, features, feature_bounds,
                                   ...
                                   contract = "random_forest", feature_cuts = NULL,
  ...
  node_privacy <- parallel::clusterCall(cluster, function() {
    caps <- dsFlower::flowerGetCapabilitiesDS()
    list(policy = dsFlower:::.privacy_policy(),
         reported = caps[grepl("^privacy_|^dp_required$|^runner_", names(caps))])
  })
  stopifnot(all(vapply(node_privacy, function(x) {
    identical(x$reported$privacy_clipping_norm, 1) &&
      identical(x$reported$privacy_unit, "row") &&
      identical(x$reported$privacy_per_training_epsilon, epsilon) &&
      identical(x$reported$privacy_per_training_delta, delta)
  }, logical(1))))
  ...
  fit <- withCallingHandlers(ds.flower.fit(
    ...
    # Public campaign diagnostics only; preserve the original failure
  ...
  spec <- dsFlowerClient:::.native_tree_release_spec(contract)
  model_path <- file.path(persisted_dir, spec$artifact_file)
  for (path in file.path(persisted_dir,
                         c("metadata.json", "history.json", spec$artifact_file,
                           spec$profile_file))) {
  ...
  list(
    ...
    metadata = metadata,
    release_profile = jsonlite::fromJSON(file.path(persisted_dir, spec$profile_file)),
    raw_history = history,

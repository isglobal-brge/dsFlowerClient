#!/usr/bin/env Rscript
# One training-only diagnostic fit; never an evaluation campaign cell.
...
input_dir <- file.path(root, "r4", "inner_seed20260820")
work <- file.path(root, "r4", "federated_default")
...
fed <- campaign_run_federated(
  ...
  rounds = 5L, model_params = list(), work_dir = work,
...
result <- list(purpose = "training-only regression diagnosis",
...
jsonlite::write_json(result, file.path(work, "diagnostic.json"),
                     pretty = TRUE, auto_unbox = TRUE, digits = NA)

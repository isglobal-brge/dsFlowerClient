"""OLS on every training row, with the frozen public regression transform."""
import argparse
import hashlib
import json
...
def fit_rows(train, protocol):
    ...
    bounds = protocol["target_bounds"]
    y = np.clip(train[protocol["target"]].to_numpy(dtype=float),
                bounds["lower"], bounds["upper"]).astype(np.float32)
    ...
    fit = dict(method="OLS", intercept=True, observation_unit="row",
               ...
               pandas_version=pd.__version__, target_transform="public clipping only")
...
def score_rows(test, protocol, coefficients, fit, training_mean):
    ...
    return dict(central=metrics(truth, design @ coefficients),

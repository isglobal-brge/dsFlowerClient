"""Custodian-installed observer for this isolated PUBLIC sequence campaign.
...
CONFIG_NAME = "sequence-public-benchmark.json"
CAMPAIGN_ROOT = Path("/workspace/cells-sequence")
...
def load_config(allowed_root=CAMPAIGN_ROOT):
    ...
    if (not isinstance(config, dict)
            ...
            or config["dataset"] not in ("uci_har",)
...
def attach(client_app, config, observer):
    ...
    if getattr(client_app._dp_fit, "_sequence_public_observer", False):
    ...
    def fit(model, X, y, pcfg, pins, n_staged, cfg, master, noise_multiplier, **kwargs):
        ...
        if cfg.get("loss-name") != "cross_entropy":
            raise RuntimeError("public observer is limited to sequence")
        ...
        payload = {"schema": "dsflower-sequence-benchmark-accountant-v1",
    ...
    fit._sequence_public_observer = True

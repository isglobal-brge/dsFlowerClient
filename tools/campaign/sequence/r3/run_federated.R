#!/usr/bin/env Rscript
# Three subject-disjoint DSLite sites; training only. TEST stays sealed.
# The released patient path averages all windows within each subject and uses
# its modal label before clipping: this does not compute a loss on every window.
...
tools <- file.path(root, "src/dsFlowerClient/tools/campaign/sequence/r3")
if (file.exists(file.path(root, "r3/test-scoring-started.json"))) {
  stop("R3 test scoring has started; refuse further training.")
}
...
stopifnot(epsilon %in% protocol$epsilon_order, seed %in% protocol$seeds,
          unit %in% protocol$units, protocol$rounds == 5L,
          unit %in% c("subject", "window"))
work <- file.path(root, "r3/runs", sprintf("%s-eps%g-seed%d", unit, epsilon, seed))
...
audit <- jsonlite::fromJSON(file.path(root, "r3/prepared/audit.json"))
train <- utils::read.csv(file.path(root, "r3/prepared/train.csv"), check.names = FALSE)

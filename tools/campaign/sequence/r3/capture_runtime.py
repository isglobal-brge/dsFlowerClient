...
result["predecessor_runtime_sha256"] = hashlib.sha256((root / "runtime.json").read_bytes()).hexdigest()
result["campaign_revision"] = "R3"
(root / "r3/runtime.json").write_text(json.dumps(result, indent=2) + "\n")
print(json.dumps({k: r[k] for k in ("dsFlower", "dsFlowerClient", "server_runner_sha256", "client_runner_sha256", "server_guard_sha256")}))

...
def fit(cfg, x, y, vx, vy, *, seed, lr, epochs, batch, optimizer, reset_every, out=None):
    if out and out.exists():
        raise RuntimeError(f"Existing central directory; refuse retraining: {out}")
    ...
    xt, yt = torch.from_numpy(x).float().cuda(), torch.from_numpy(y).long().cuda()
    vt = torch.from_numpy(vx).float().cuda()
    history = []
...
def main(root, resume=False):
    ...
    audit = {"started_at": datetime.now(timezone.utc).isoformat(), "test_accessed": False,
             "train_subjects": ids[~np.isin(ids, val_ids)].tolist(),
    ...
    records = json.loads((out / "experiments.json").read_text()) if resume else {}
    # Frozen, small TRAIN-only schedule comparison; do not use test outcomes.
    for lr in (.003, .01):
        if f"public_adam_{lr}" in records:
            continue
...
if __name__ == "__main__":
    ...
    p.add_argument("--resume-diagnosis", action="store_true")
    args = p.parse_args()
    main(args.root, args.resume_diagnosis)

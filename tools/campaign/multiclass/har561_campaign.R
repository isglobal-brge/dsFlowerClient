# HAR561 adaptation of campaign_lib.R: patient privacy and six target levels.
# The test argument remains lazy until the scoring callback after training.
...
campaign_run_federated <- function(site_data, test, features, feature_bounds,
                                   ...
                                   contract = "pytorch_multiclass",
                                   target = "target", score_function) {
  ...
  node_privacy <- parallel::clusterCall(cluster, function() {
    list(reported_policy = dsFlower::flowerPrivacyPolicyDS(),
         contract = dsFlower:::.privacy_policy(),
         clipping_norm = dsFlower:::.serverDpClippingNorm(),
         runner_sha256 = dsFlower:::.compute_harness_hash())
  })
  names(node_privacy) <- names(conns)
  stopifnot(all(vapply(node_privacy, function(node) {
    policies <- list(node$reported_policy, node$contract)
    all(vapply(policies, function(policy) {
      identical(policy$dp_unit, "patient") &&
        identical(policy$patient_column, "subject") &&
        identical(policy$adjacency, "replace_one") &&
        isTRUE(all.equal(policy$per_training_epsilon, epsilon)) &&
        isTRUE(all.equal(policy$per_training_delta, delta))
    }, logical(1))) && isTRUE(all.equal(node$clipping_norm, 1))
  }, logical(1))))
  ...
  fit <- withCallingHandlers(ds.flower.fit(
    ...
    target_levels = as.character(1:6),
    torch_backend = "cpu",
    output_dir = output_dir,
    silent = TRUE,
    verbose = FALSE
  ), error = function(e) {
    # Public multiclass campaign diagnostics; preserve the original failure
  ...
  # Channel B: score the released artifact once on the held-out test set.

...
image_size_lit <- as.integer(validation$image_size)
...
model_call_lines <- function(method, image_size) {
  switch(
    method,
    pytorch_resnet18 = c(
      "model <- ds.flower.model.pytorch_resnet18(",
      "  n_classes = 2L, learning_rate = 0.001,",
      paste0("  batch_size = 2L, local_epochs = 1L, image_size = ", image_size, "L"),
      ")"
    ),
    pytorch_densenet121 = c(
    ...
    pytorch_unet2d = c(
      ...
      paste0("  local_epochs = 1L, image_size = ", image_size, "L, base_channels = 8L"),
      ")"
    ),
    stop("Unknown vision method: ", method, call. = FALSE)
  )
}
...
method_vignette <- function(method) {
  ...
  target_lit <- paste0("'", row_meta$target, "'")
  rounds_lit <- as.integer(row_meta$rounds)
  model_lines <- model_call_lines(method, image_size_lit)
  secondary_metric_line <- if (identical(method, "pytorch_unet2d")) {
    "  centralized_dice = row$centralized_dice,"
  } else {

...
For a larger public direct-image benchmark, `inst/demos/benchmark_pathmnist_direct_image.R`
uses a balanced PathMNIST binary subset from MedMNIST v2. It writes PNG files to
the Rock-side image root, uploads only metadata and relative paths to Opal,
trains a centralized ResNet-18 baseline, then runs the federated ResNet-18
template under `trusted_internal` and `consortium_internal`:
...
Validation run on 2026-05-27: three Opal/Rock clients, 1,500 PathMNIST images,
two federated rounds, zero client failures, centralized accuracy `0.9947` vs
federated checkpoint accuracy `0.9860`. The `consortium_internal` branch

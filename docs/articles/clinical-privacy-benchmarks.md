...
reproduce the benchmark, then the recorded results loaded from the
committed evidence artifacts. No live Opal servers are required to
render this page; the command chunks are shown for reference and are not
executed.

## Running This Benchmark

Each dataset is partitioned across three Opal nodes that are already set
up and serving the cohort table; each benchmark row is one
[`ds.flower.fit()`](https://isglobal-brge.github.io/dsFlowerClient/reference/ds.flower.fit.md)
...
three Opal nodes that hold the partitioned cohort (the CDC Diabetes
Health Indicators table is shown).
...
**2. Run a benchmark row.** Train one authorised template through
dsFlower with FedAvg over the server-side table, choosing the privacy
profile for the posture under test. The clinical Secure Aggregation
...
Swapping the `privacy` argument moves between postures:
`"trusted_internal"` for ordinary federated training,
`"clinical_default"` for the Secure Aggregation profile over the
`sklearn_logreg`, `sklearn_ridge`, `sklearn_sgd`, `pytorch_logreg`,
`pytorch_mlp` and secure `xgboost_histogram` templates,
`"high_sensitivity_dp"` for Opacus DP-SGD on the compatible PyTorch
templates, and `"clinical_update_noise"` for the bounded-noise XGBoost
histogram route. The epsilon curves below repeat the DP and update-noise
rows across a sweep of requested budgets.

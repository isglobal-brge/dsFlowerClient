...
This run trained the xgboost template on the classification task (target
outcome, 12 synthetic features) for 1 federated round(s) under the
sandbox_open profile, across 3 Opal/Rock nodes holding the same pooled
cohort used for the centralized baseline.

The centralized baseline reached a log_loss of 0.4348; the federated run
reached 0.6931 with NA client failures, leaving this template **pass**.
...
xgboost.](validation-xgboost_files/figure-html/unnamed-chunk-2-1.png)

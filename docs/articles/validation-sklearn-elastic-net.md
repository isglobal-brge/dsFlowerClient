...
This run trained the sklearn_elastic_net template on the classification
task (target outcome, 12 synthetic features) for 1 federated round(s)
under the sandbox_open profile, across 3 Opal/Rock nodes holding the
same pooled cohort used for the centralized baseline.

The centralized baseline reached a log_loss of 0.1965; the federated run
reached 0.2000 with 0 client failures, leaving this template **pass**.
The acceptance comparison is shown below, followed by the centralized
vs. federated loss.
...
sklearn_elastic_net.](validation-sklearn-elastic-net_files/figure-html/unnamed-chunk-2-1.png)

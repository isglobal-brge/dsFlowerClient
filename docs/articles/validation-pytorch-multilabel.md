...
fit <- ds.flower.fit(
  conns,
  symbol   = 'D',
  target   = c('label_a', 'label_b', 'label_c'),
  features = paste0('x', 1:12),
  model    = 'pytorch_multilabel',
  strategy = 'fedavg',
...
This run trained the pytorch_multilabel template on the classification
task (target label_a,label_b,label_c, 12 synthetic features) for 1
federated round(s) under the sandbox_open profile, across 3 Opal/Rock
nodes holding the same pooled cohort used for the centralized baseline.

The centralized baseline reached a multilabel_bce of 0.5933; the
federated run reached 0.6503 with 0 client failures, leaving this
...
pytorch_multilabel.](validation-pytorch-multilabel_files/figure-html/unnamed-chunk-2-1.png)

# Validation: pytorch_cause_specific_cox
...
| Field                      | Value                      |
|:---------------------------|:---------------------------|
| method                     | pytorch_cause_specific_cox |
| task                       | survival                   |
| target                     | time,event_type            |
| n_features                 | 12                         |
| rounds                     | 1                          |
| centralized_metric         | cause_specific_cox_loss    |
| centralized_loss           | 4.6180                     |
| centralized_accuracy       | NA                         |
| federated_status           | ok                         |
| federated_loss             | 2.6028                     |
...
pytorch_cause_specific_cox.](validation-pytorch-cause-specific-cox_files/figure-html/unnamed-chunk-3-1.png)

## Step-By-Step Run Output

The following chunks print the audit trail for the committed catalogue
coverage run. They are evaluated when pkgdown renders the vignette, so
the page shows concrete centralized and federated outputs rather than
only describing the API shape.
...
| 3\. Select task target | time, event_type |
...
| 2\. Train centralized baseline | status = ok \| metric = cause_specific_cox_loss |
| 3\. Evaluate centralized baseline | loss = 4.6180 \| accuracy = NA |
...
#> [centralized] method: pytorch_cause_specific_cox
#> [centralized] task: survival | target: time,event_type | features: x1..x12
#> [centralized] status: ok
#> [centralized] cause_specific_cox_loss: 4.6180
...
| 2\. Prepare server-side dsFlower nodes | template = pytorch_cause_specific_cox \| privacy = sandbox_open |
| 3\. Start Flower SuperLink/SuperNodes | federated_run_id = 15036085043497125280 |
| 4\. Run federated training | rounds = 1 \| federated_loss = 2.6028 \| client_failures = 0.0000 |
...
#> [dsFlower] model: pytorch_cause_specific_cox | strategy: fedavg | rounds: 1
#> [dsFlower] run id: 15036085043497125280
#> [Flower] aggregate_fit/evaluate: clients=3 failures=0.0000
#> [Flower] final federated loss: 2.6028
#> [cleanup] status: ok
```
...
|---:|---:|---:|---:|:---|:---|
| 4.618 | 2.6028 | 11.7951 | 0 | TRUE | pass |

## Inline Execution Path

The validation row above was generated by running this method through
the same inline DataSHIELD/Flower flow shown below. The rendered result
is loaded from the committed evidence artifact written by that run.

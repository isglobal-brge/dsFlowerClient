...
fit <- ds.flower.fit(
  conns,
  symbol   = 'D',
  target   = c('time', 'event_type'),
  features = paste0('x', 1:12),
  model    = 'pytorch_cause_specific_cox',
  strategy = 'fedavg',
...
This run trained the pytorch_cause_specific_cox template on the survival
task (target time,event_type, 12 synthetic features) for 1 federated
round(s) under the sandbox_open profile, across 3 Opal/Rock nodes
holding the same pooled cohort used for the centralized baseline.

The centralized baseline reached a cause_specific_cox_loss of 4.6180;
the federated run reached 2.6028 with 0 client failures, leaving this
...
pytorch_cause_specific_cox.](validation-pytorch-cause-specific-cox_files/figure-html/unnamed-chunk-2-1.png)

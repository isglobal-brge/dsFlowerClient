...
This run trained the pytorch_lognormal_aft template on the survival task
(target time,event, 12 synthetic features) for 1 federated round(s)
under the sandbox_open profile, across 3 Opal/Rock nodes holding the
same pooled cohort used for the centralized baseline.

The centralized baseline reached a lognormal_aft_nll of 3.4966; the
federated run reached 3.6834 with 0 client failures, leaving this
...
pytorch_lognormal_aft.](validation-pytorch-lognormal-aft_files/figure-html/unnamed-chunk-2-1.png)

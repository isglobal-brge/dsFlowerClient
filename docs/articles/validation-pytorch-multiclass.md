...
This run trained the pytorch_multiclass template on the classification
task (target class3, 12 synthetic features) for 1 federated round(s)
under the sandbox_open profile, across 3 Opal/Rock nodes holding the
same pooled cohort used for the centralized baseline.

The centralized baseline reached a cross_entropy of 0.9693; the
federated run reached 1.1560 with 0 client failures, leaving this
...
pytorch_multiclass.](validation-pytorch-multiclass_files/figure-html/unnamed-chunk-2-1.png)

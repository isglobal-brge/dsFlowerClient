...
This run trained the pytorch_coxph template on the survival task (target
time,event, 12 synthetic features) for 1 federated round(s) under the
sandbox_open profile, across 3 Opal/Rock nodes holding the same pooled
cohort used for the centralized baseline.

The centralized baseline reached a cox_partial_likelihood of 4.8611; the
federated run reached 2.7456 with 0 client failures, leaving this
...
pytorch_coxph.](validation-pytorch-coxph_files/figure-html/unnamed-chunk-2-1.png)
